...
#
def extract_dwt_features(signals, waveletname,level):
    all_features = []
...
X_train_arr_fft = extract_fft_features(train_signals, T, N, f_s, denominator)
X_test_arr_fft = extract_fft_features(test_signals, T, N, f_s, denominator)

waveletname = 'sym2' 
X_train_arr_dwt = extract_dwt_features(train_signals, waveletname,level)
X_test_arr_dwt = extract_dwt_features(test_signals, waveletname,level)

X_train_fft = create_df_features(X_train_arr_fft,'FFT')
X_test_fft  = create_df_features(X_test_arr_fft,'FFT') 
X_train_fft_scaled = scale_df_features(X_train_fft)
X_test_fft_scaled = scale_df_features(X_test_fft)
...
X_train_scaled = pd.concat([X_train_fft_scaled,X_train_dwt],axis=1)
X_test_scaled = pd.concat([X_test_fft_scaled,X_test_dwt],axis=1)

y_train_df = pd.DataFrame(y_train,columns=['label'])
y_test_df = pd.DataFrame(y_test,columns=['label'])

save_df_file(PATH_DATA, 'X_train.csv', X_train)
save_df_file(PATH_DATA, 'X_test.csv', X_test)
save_df_file(PATH_DATA, 'y_train.csv', y_train_df)
save_df_file(PATH_DATA, 'y_test.csv', y_test_df)
