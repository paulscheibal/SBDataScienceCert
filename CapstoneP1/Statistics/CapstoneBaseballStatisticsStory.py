...
battingf = path + 'dfbatting_player_cstats.csv'
...
mu = np.mean(data)
sigma = np.std(data)
ptiles_val = np.percentile(data,[2.5,97.5])
...
label1 = '2.5%% CI - %1.3f ' % ptiles_val[0]
label2 = '97.5%% CI - %1.3f ' % ptiles_val[1]
plb.axvline(ptiles_vals[0],label=label1, color='#ff9999')
plb.axvline(ptiles_vals[1],label=label2, color='#ff9999')
...
farr, corrarr, rsqarr = plot_poly_r2(syr,sops,degreearr, colorarr,3)
ax.yaxis.set_major_formatter(mtick.StrMethodFormatter('{x:1.3f}'))
ax.tick_params(axis='both', which='major', labelsize=12)
plt.xticks(rotation=45)
leg = plt.legend()
...
for i in range(0,len(farr)):
    print('Degree ' + str(degreearr[i])  + ', Sum of Squared Diff %1.4f' % rsqarr[i] )
bestfit_index = rsqarr.index(min(rsqarr))
print('Best RSquared Fit is ' + str(degreearr[bestfit_index]))
