...
def get_players(df,n,m,thresholdOPS):
    ...
    if n < 0 :
        ...
        sublst = playerlst[subidx]
    return sublst

def drop_minyears(df,years):
    plst = df['playerID'].drop_duplicates()
    plst = np.array(plst)
    df = df.sort_values(['playerID','yearID'],axis=0)
    finaldf = pd.DataFrame()
    for p in plst:
        dftemp = df[(df['playerID'] == p)].tail(years)
        finaldf = finaldf.append(dftemp)
    finaldf = finaldf.reset_index(drop=True)  
    return finaldf
        
def starting_age(df,minage,maxage):
    dfp = df[['playerID','age']]
    dfp = dfp.groupby('playerID').min()
    dfp = dfp.reset_index(drop=False)
    dfp = dfp[ (dfp['age'] >= minage) & (dfp['age'] <= maxage)]
    dfp = np.array(dfp.playerID)
    df = df[df['playerID'].isin(dfp)]
    print(dfp)
    return df
...
#df = normalize_values(df,['lag1_OPS','lag1_cOPS','yearID','height','weight','lag1_H','lag1_cH','lag1_HR','lag1_cHR','lag1_AB','lag1_AVG','lag1_cAVG'],['lag1_nOPS','lag1_ncOPS','nyearID','nheight','nweight','lag1_nH','lag1_ncH','lag1_nHR','lag1_ncHR','lag1_nAB','lag1_nAVG','lag1_ncAVG'],'zeromean')

feature_list =   ['age','POS_1B','POS_2B','POS_3B','POS_SS','POS_OF','lag1_OPS','lag1_cOPS','lag1_H','lag1_cHR']
...
playerlistx = get_players(df_test,samplesize,minyearsplayed,OPSAVG_threshold)
...
df_ptp_f = starting_age(df_ptp_f,minage_start,maxage_start)
df_ptp_f = drop_minyears(df_ptp_f,predyears)

#df_ptp_f = normalize_values(df_ptp_f,['lag1_OPS','lag1_cOPS','yearID','height','weight','lag1_H','lag1_cH','lag1_HR','lag1_cHR','lag1_AB','lag1_AVG','lag1_cAVG'],['nlag1_OPS','nlag1_cOPS','nyearID','nheight','nweight','lag1_nH','lag1_ncH','lag1_nHR','lag1_ncHR','lag1_nAB','lag1_nAVG','lag1_ncAVG'],'minmax')
...
degree = 2
model = make_pipeline(Normalizer(),PolynomialFeatures(degree), LinearRegression())
